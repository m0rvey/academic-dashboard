...
def run_gui(db: IDatabaseManager) -> None:
    ...
    async def main(page: ft.Page) -> None:
        ...
        # Механизм debounce:
        # Используем call_later для отложенного вызова _safe_refresh (через 300мс).
        # Если до истечения таймера поступают новые события изменения файлов (от watchdog),
        # предыдущий таймер отменяется и запускается новый.
        # Это предотвращает мерцание и лавинообразные перерисовки UI при серии мелких изменений БД.
        # Дополнительно asyncio.Lock() гарантирует, что само обновление не запустится параллельно.
